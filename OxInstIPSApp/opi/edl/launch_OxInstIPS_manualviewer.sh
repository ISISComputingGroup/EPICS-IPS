#!/bin/sh
#
# To launch a pdf viewer on the Oxford Instruments manuals in the
# documentation directory.  A script which works when run from the
# source code location or the built code location over different
# releases of the module without using the problematic TOPDIR variable
# or unix .. path in the contect of the EPICS build.  We know that we
# in this OxInstIPS script here we are somewhere below the
# OxInstIPS directory, and the documentation directory is just below
# it, therefore we share a common parent directory somewhere up the
# tree with the OxInstIPS documentation directory, so find out our
# path and go up it until we find the documentation directory.
#
...
relative_manual_dirname="private/Manufacturer/OxfordInstruments"
rootdir="/"
...
absolute_OxInstIPS_dirpath=$my_absolute_dirpath
absolute_doc_dirname=""

while [ "$absolute_OxInstIPS_dirpath" != "$rootdir" ]; do

  # Construct possible absolute documentation directory name.
  absolute_doc_dirname=$absolute_OxInstIPS_dirpath/$relative_doc_dirname
...
#  echo "absolute_doc_dirname: $absolute_doc_dirname"
  ...
  if [ -d $absolute_doc_dirname ]; then
    ...
    if [ -d $absolute_manual_dirname ]; then
      ...
      absolute_OxInstIPS_dirpath=$rootdir
    else
  ...
  absolute_OxInstIPS_dirpath=$(dirname $absolute_OxInstIPS_dirpath)
done
